...
const EmployeeList = () => {
  ...
  const fetchEmployees = async () => {
    ...
    try {
      const res = await axios.get("https://springbootdeploy-p2lu.onrender.com/employee", {
        headers: {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const res = await axios.get(`https://springbootdeploy-p2lu.onrender.com/employee/search/${encodeURIComponent(searchTerm)}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleDelete = async (id) => {
    const token = localStorage.getItem("token");
    try {
      await axios.delete(`https://springbootdeploy-p2lu.onrender.com/employee/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
